...
from wrap_ew2.trace_diff import (
...
def test_trace_diff_helper_identifies_tick_kind_mismatch(tmp_path: Path) -> None:
    ...
    events_a = [
        {"tick": 0, "intent": {"op": "harvest"}},
        {"tick": 1, "intent": {"op": "transfer"}},
    ]
    events_b = [
        {"tick": 0, "intent": {"op": "harvest"}},
        {"tick": 1, "intent": {"op": "warn"}},
    ]
    assert first_tick_kind_mismatch(events_a, events_b)["kind_b"] == "warn"
    print_pair_or_stop("fixture", result)


def test_trace_diff_identical_says_identical_and_stops(tmp_path: Path) -> None:
    path = tmp_path / "same.jsonl"
    path.write_text('{"tick":0,"intent":{"op":"harvest"},"seed":42}\n', encoding="utf-8")
    result = compare_jsonl_traces(path, path)
    line = print_pair_or_stop("self", result)
...
def test_different_seed_same_arm_different_sha(tmp_path: Path) -> None:
    ...
    result = compare_jsonl_traces(a, b)
    print_pair_or_stop("42vs7", result)
...
def test_wrap_seeds_42_7_99_first_tick_kind_mismatch(tmp_path: Path) -> None:
    ...
    for left, right in ((42, 7), (42, 99)):
        ...
        print_pair_or_stop(f"{left}vs{right}", result)
        if result["status"] == "IDENTICAL":
            pytest.fail(
        ...
        if result["mismatch"] is not None:
            print(
                f"{left}vs{right} first tick/kind mismatch: "
                f"kind_a={result['mismatch']['kind_a']} "
                f"tick_a={result['mismatch']['tick_a']} "
                f"kind_b={result['mismatch']['kind_b']} "
                f"tick_b={result['mismatch']['tick_b']}"
            )
        else:
            print(f"{left}vs{right} first tick/kind mismatch: none (kind+tick sequences match)")
